...
function totalVideosDuration(channel) {
  return channel.videos.reduce((sum, video) => (sum += video.duration), 0);
}
...
function channelWithMostContent(channels) {
  return channels.sort(
    (channel) => channel.videos.duration >= totalVideosDuration
  )[0];
}
...
function longestChannelName(channels) {
  return channels.sort(
    (channel1, channel2) => channel2.name.length - channel1.name.length
  )[0];
}
